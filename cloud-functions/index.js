...
exports.getGroupDetails = functions.https.onCall(async (data, context) => {
  try {
    ...
    // Get group data
    const groupDoc = await firestore.collection('groups').doc(data.data.groupId).get();
    ...
    const isMember = groupData.members.some(member => member.id === data.auth.uid);
...
exports.initializeShoppingLists = functions.https.onCall(async (data, context) => {
  try {
    // Ensure authenticated
    if (!data.auth) {
      throw new functions.https.HttpsError(
...
exports.sendInvitation = functions.https.onCall(async (data, context) => {
  try {
    ...
    const isMember = groupData.members.some(member => member.id === inviterId);
    ...
    const isAlreadyMember = groupData.members.some(member => member.id === inviteeId);
    ...
    const hasInvite = (inviteeData.invites || []).some(invite => invite.groupId === data.data.groupId);
    ...
    // Create invitation
    const inviteId = firestore.collection('invites').doc().id; // Generate a unique ID
    const invitation = {
      id: inviteId,
...
exports.acceptInvitation = functions.https.onCall(async (data, context) => {
  try {
    ...
    // Find the invitation
    const invitation = (userData.invites || []).find(invite => invite.id === data.data.inviteId);
    ...
    // Get group data
    const groupDoc = await firestore.collection('groups').doc(invitation.groupId).get();
    ...
    await firestore.runTransaction(async (transaction) => {
      ...
      const isAlreadyMember = groupData.members.some(member => member.id === userId);
